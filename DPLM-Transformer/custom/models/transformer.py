    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _ = layer(
                ...
                need_decoder_self_attention=bool((idx == alignment_layer)) #!Here, we add a parameter in transformer_layer, to get the decoder self-attention
            )
            inner_states.append(x)
            if layer_attn is not None and idx == alignment_layer:
                attn = layer_attn.float().to(x)
        #! Here, attn size is [batch_size, seq_q, seq_q], it's a wrong code to write "attn = attn.mean(dim=0)", so we annotate following codes

        # print("attn is : ", attn)
        # print("idx is : ", attn.size(-1))
        # if attn is not None:
        #     if alignment_heads is not None:
        #         attn = attn[:alignment_heads]
        #     # average probabilities over heads
        #     attn = attn.mean(dim=0)
        # print("in transformer size is : ", attn.size())
        if self.layer_norm is not None:
            x = self.layer_norm(x)
